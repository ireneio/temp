...
export default React.memo(({ store, collapsed, isNotOpened }: PropsType) => {
  const { t } = useTranslation('common');
  const { selectedKeys, menuList } = useFooterMenuList(store);
      ...
      {collapsed ? (
        ...
        menuList.map(({ key, Icon, url, target }) => (
          <Link
            ...
            href={url}
            target={target}
            disabled={isNotOpened && !['signout', 'bill-payment'].includes(key)}
          >
            <a
